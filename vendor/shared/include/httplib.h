...
{
  public:
  std::string QueryString, Path, Protocol, Host, Port, User, Password;
  ...
  static Uri Parse(const std::string &uri)
  {
      ...
      // pass & user
      iterator_t userStart = protocolEnd;
      iterator_t userEnd = std::find(userStart, uriEnd, '@');
      if(userEnd != uriEnd) {
          //std::cout << std::string(userStart, userEnd) << std::endl;
          iterator_t passStart = std::find(userStart, userEnd, ':');
          if(passStart != userEnd) {
              result.User = std::string(userStart, passStart);
              result.Password = std::string(++passStart, userEnd);
              protocolEnd = ++userEnd;
              //std::cout << result.User << std::endl << result.Password << std::endl;
          }
      }
